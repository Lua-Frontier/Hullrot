...
#
ROBUST_CDN_URL = "https://cdn.luacorp.ru/"
FORK_ID = "hullrot"
...
def get_engine_version() -> str:
    branch = subprocess.run(["git", "rev-parse", "--abbrev-ref", "HEAD"], stdout=subprocess.PIPE, cwd="RobustToolbox", check=True, encoding="UTF-8").stdout.strip()
    commit = subprocess.run(["git", "rev-parse", "--short", "HEAD"], stdout=subprocess.PIPE, cwd="RobustToolbox", check=True, encoding="UTF-8").stdout.strip()
